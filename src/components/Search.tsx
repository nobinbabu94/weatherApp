...
const Search = ({
  ...
  return (
    ...
    >
      ...
      <ul ref={suggestionRef} className="z-10 relative bg-white flex w-[14em] flex-col items-start rounded-lg ">
        {suggestions.map((item: suggestionType, index: number) => (
          <li
            key={item.name + index}
            onClick={() => suggestionData(item)}
            className="text-black bg-gray-100 cursor-pointer w-full p-1 rounded-b-lg
             hover:bg-slate-200 flex justify-start pl-4"
          >
